...
def Initial_motor_position(Motor_type,Bottom_length):
    ...
    if 'TypeB' in Motor_type:
        bpy.ops.transform.rotate(value=1.5708, axis=(1, 0, 0), constraint_axis=(True, False, False), 
                        constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
        bpy.ops.transform.translate(value=(0,-0.03,0), constraint_axis=(False, False, False),
                    constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
    ...
    if 'TypeA2' not in Motor_type and 'TypeA' in Motor_type:
        bpy.ops.transform.translate(value=(-0.02 ,0 , 0), constraint_axis=(False, False, False),
                    constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
    bpy.ops.transform.translate(value=(random.uniform(-0.925, -0.875)+Bottom_length, 0.2, 1.14), constraint_axis=(False, False, False),
                    constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
...
def cut(data_to_cut,noise,cut,camera_position_now):
    ...
    noise_xmin = random.uniform(-0.4, 0.05)
    noise_xmax = random.uniform(-0.2, 0.25)
    noise_y = random.uniform(-0.15, 0.15)
    if cut:
        Corners = [(-1.7+noise_xmin, -0.23+noise_y, 1.4),(-0.27+noise_xmax, -0.23+noise_y, 1.4),
                   (-0.27+noise_xmax, 0.68+noise_y, 1.4),(-1.7+noise_xmin, 0.68+noise_y, 1.4),
                   (-1.7+noise_xmin, -0.23+noise_y, cly_bottom),(-0.27+noise_xmax, -0.23+noise_y, cly_bottom),
                   (-0.27+noise_xmax, 0.68+noise_y, cly_bottom),(-1.7+noise_xmin, 0.68+noise_y, cly_bottom)]
    else:
    ...
    for corner in Corners:
        cor_inCam_point = get_Corordinate_inCam(camera_position_now[0], camera_position_now[1], camera_position_now[2],
                                                camera_position_now[3], camera_position_now[4], camera_position_now[5], corner)
...
def main(num_modles):
    ...
    for Motor_type in list_types_motor:
        ...
        for dirs in List_motor :
            ...
            Initial_motor_position(Motor_type, BottomLength)
